...
maxlag2=5;
maxlag3=10;
akf = xcorr(x,x,'unbiased');
akf_max1=xcorr(x,x,maxlag1, 'unbiased');
akf_max2=xcorr(x,x,maxlag2, 'unbiased');
akf_max3=xcorr(x,x,maxlag3, 'unbiased');
figure(2)
subplot (4, 1, 1); plot (akf); grid on;
subplot (4, 1, 2); plot (akf_max1); grid on;
subplot (4, 1, 3); plot (akf_max2); grid on;
subplot (4, 1, 4); plot (akf_max3); grid on;
...
n = 512; 
p=3.1416
t = [100:1000];		% вектор часу
h=t*2*p*f0;
s =sin(h);
v = sqrt(D)*randn(1,length(t));  
x=s+v;  
maxlag1=1;
akf_un=xcorr(x,x,maxlag1, 'biased');
akf_bias=xcorr(x,x,maxlag1, 'unbiased');
figure(4)
